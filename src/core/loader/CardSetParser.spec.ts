...
describe("Card Set Parser", function () {
    ...
    it("parses a set with a multiple cards and title", function () {
        ...
        expect(card1.sides[0].raw.title).to.be.equal("Side A1");
        expect(card1.sides[1].raw.title).to.be.equal("Side B1");
        
        const card2 = cardSet.card(1);
        expect(card2).to.be.not.undefined;
        expect(card2.sides[0].raw.title).to.be.equal("Side A2");
        expect(card2.sides[1].raw.title).to.be.equal("Side B2");
        
        const card3 = cardSet.card(2);
        expect(card3).to.be.not.undefined;
        expect(card3.sides[0].raw.title).to.be.equal("Side A3");
        expect(card3.sides[1].raw.title).to.be.equal("Side B3");
    ...
    it("parses a set with a multiple cards with details and title", function () {
        ...
        expect(card1).to.be.not.undefined;
        expect(card1.sides[0].raw.title).to.be.equal("Side A1");
        expect(card1.sides[0].raw.body).to.be.equal("Card 1 Side A");
        expect(card1.sides[1].raw.title).to.be.equal("Side B1");
        expect(card1.sides[1].raw.body).to.be.equal("Card 1 Side B");
        
        const card2 = cardSet.card(1);
        expect(card2).to.be.not.undefined;
        expect(card2.sides[0].raw.title).to.be.equal("Side A2");
        expect(card2.sides[0].raw.body).to.be.equal("Card 2 Side A");
        expect(card2.sides[1].raw.title).to.be.equal("Side B2");        
        expect(card2.sides[1].raw.body).to.be.equal("Card 2 Side B Line 1\nCard 2 Side B Line 2");
        
        const card3 = cardSet.card(2);
        expect(card3).to.be.not.undefined;
        expect(card3.sides[0].raw.title).to.be.equal("Side A3");
        expect(card3.sides[0].raw.body).to.be.equal("Card 3 Side A Line 1\nCard 3 Side A Line 2\nCard 3 Side A Line 3");
        expect(card3.sides[1].raw.title).to.be.equal("Side B3");
        expect(card3.sides[1].raw.body).to.be.equal("Card 3 Side B Line 1\nCard 3 Side B Line 2");
    ...
    it("ignores additional level 1 headings before cards", function () {
        ...
        expect(card1).to.be.not.undefined;
        expect(card1.sides[0].raw.title).to.be.equal("Side A1");
        expect(card1.sides[0].raw.body).to.be.equal("");
        expect(card1.sides[1].raw.title).to.be.equal("Side B1");
        expect(card1.sides[1].raw.body).to.be.equal("# Another l1 heading");
    });
    ...
    it("trims whitespaces at start and end", function () {
        ...
        expect(card2.sides[0].raw.body).to.be.equal("Card 2 Side A");
        expect(card2.sides[1].raw.title).to.be.equal("Side B2");
        expect(card2.sides[1].raw.body).to.be.equal("Card 2 Side B");
    });
    ...
    it("ignores dangling level 3 heading", function () {
        ...
        expect(card1.sides[0].raw.body).to.be.equal("");
        expect(card1.sides[1].raw.title).to.be.equal("Side B1");
        expect(card1.sides[1].raw.body).to.be.equal("");
    });
});
